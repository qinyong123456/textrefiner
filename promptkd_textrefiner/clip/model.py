    ...
    def forward(self, x: torch.Tensor, all_layer_outputs=False):
        ...
        if not all_layer_outputs:
            ...
            for i, block in enumerate(self.transformer.resblocks):
                ...
                cur_output = self.ln_post(cur_output)
            
                if self.proj is not None:
                    cur_output = cur_output @ self.proj
                if block.add_prompt or i > self.prompt_till_layer_visual or i == 0:
                    feature_maps.append((cur_output[:,0 : x.shape[0] - self.n_ctx_visual,:], attn[:, 0, 1:x.shape[0] - self.n_ctx_visual]))
                else:
                    feature_maps.append((cur_output[:,:,:], attn[:, 0, 1:]))
            
            return cur_output[:,0,:], feature_maps
